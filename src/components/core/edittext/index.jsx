...
class TextEditor extends React.PureComponent {
  ...
  onUpload = (path) => {
    this.editor?.insertHtml(
      `<p><img src="${path}" class='h-auto' style="width: 400px"/></p>`
    );
    setTimeout(() => {
  ...
  onChoose = (event) => {
    const { cb, errorMessage, fileType } = this.props;
  ...
  componentDidUpdate() {
    ...
    if (
      editor &&
      editor.getData() !== this.props.defaultValue &&
      !this.isOnChange
    ) {
  ...
  render() {
    return (
      <>
        <div className="shd-content-editor" id={this.nameCKEditor}></div>
