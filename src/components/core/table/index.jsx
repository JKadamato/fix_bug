...
class UITable extends React.Component {
  ...
  componentDidMount() {
    const { staticData, staticHeader, location, isAutoFetchData, isAddParams } =
      this.props;
  ...
  componentDidUpdate(prevProps, prevState) {
    const { search, isReload, staticData, columns, staticHeader, queries } =
      this.props;
    ...
    if (queries && !_.isEqual(queries, prevProps.queries)) {
      this.changeStateFetchData(search, true);
    }
  ...
  changeStateFetchData = (search, resetQuerParams = false) => {
  ...
  fetchTableData = () => {
    const { limit, page, sort, search } = this.state;

    const { service, columns, payload, queries } = this.props;
        ...
        if (!hiddenCols[newColumns[i].value]) {
          definedColumns = [
            ...definedColumns,
            {
              ...
              sort: this.definePositionOfColumn(
                newColumns[i]?.sort,
                columns,
                i,
              ),
  ...
  makeColumnName = (column) => {
    const { customColsName } = this.props;
  ...
  hiddenColumn = (oldColumns) => {
    ...
    for (let i = 0; i < oldColumns.length; i++) {
      if (hiddenCols.filter((cl) => cl === oldColumns[i].value).length === 0) {
        newColumns = [...newColumns, oldColumns[i]];
      }
    }
  ...
  makecustomCols = (headers) => {
    const { defineCols, headerWidth } = this.props;
    ...
    for (let i = 0; i < defineCols.length; i++) {
      definedcustomCols = [
        ...definedcustomCols,
        {
          title: !this.isFunction(defineCols[i].name)
            ? defineCols[i].name
            : defineCols[i].name(rows),
  ...
  onChangePage = (page) => {
    const { staticData, isAddParams } = this.props;
  ...
  convertArrayColumnsToAntdColumns = () => {
    ...
    return _.orderBy(
      [...this.makeColumns(columns), ...this.makecustomCols(columns)],
      ["sort"],
      ["asc"],
    );
  };
  ...
  render() {
    const { pagination, loading, limit, page, rows, total } = this.state;

    const { className, isHiddenPg, onSelectAll, rowKey } = this.props;
