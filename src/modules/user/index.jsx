...
const Users = ({ global: { user } }, isPremium) => {
  ...
  const formatDate = (value) => {
    const dateObject = new Intl.DateTimeFormat("en-GB", {
      year: "numeric",
      month: "long",
      day: "2-digit",
    });
    return dateObject.format(new Date(value));
  };
      ...
      <div className="core-card">
        <div className="flex justify-between">
          {userIds.length === 0 ? (
            ...
            isAllow && (
              <UIButton onClick={isAllow && removeUsers} className="third">
                Remove user
              </UIButton>
            )
          )}
            ...
            customComp={{
              status: ({ text, row }) => (
                <Tag
                  className="uppercase"
                  style={{ width: 110 }}
                  type={row?.premium ? "primary" : "second"}
                >
                  {!row?.isPremium ? "Free" : "Premium"}
                </Tag>
              ),
              action: ({ row }) => (
              ...
              email: ({ text, row }) => (
                <div className="lowercase text-left">
                  {isAllow ? (
                    ...
                    <div className="base-text" style={{ color: "#2a2a2c" }}>
              ...
              totalDownload: ({ text }) => (
                <div className="">{formatMB(text || 0)}</div>
              ),
              totalUpload: ({ text }) => (
                <div className="">{formatMB(text || 0)}</div>
              ),
              createdAt: ({ text }) => (
                <div className=""> {formatDate(text) || "Never"}</div>
              ),
              subscription: ({ text }) => (
                <div className="">{text?.subscriptionType || "Free"}</div>
              ),
            }}
