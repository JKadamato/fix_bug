...
const ResetPass = (props) => {
  ...
  const [isValidCode, setValid] = React.useState(false);
  const fields = {
  ...
  React.useEffect(() => {
    ...
    checkCode(currentQuery?.code);
  }, []);

  const checkCode = (code) => {
    GET(`/admin/check-code/${code}`, {}, true)
      .then(() => {
        setValid(true);
      })
      .catch(() => {
        ...
        setMessage({
          isError: true,
          text: "Error occurred during confirmation. The code might have expired or the password is too weak",
        });
      });
  };
    ...
    if (currentQuery?.code) {
      POST(`/admin/change-password`, {
        ...
        .then((result) => {
          setMessage({
            isError: false,
            text: "Password reset has been confirmed and new password updated",
          });
          setTimeout(() => {
  ...
  return (
    <Layout
      ...
      className="login flex ustify-center h-screen"
    >
      <Container>
        <Row>
          ...
          <Col className="col-sm-6">
            <div className="login-content">
              ...
              >
                {(form) => {
                  ...
                  return (
                    <Form onFinish={(e) => isValidCode && handleSubmit(e)}>
                      ...
                      <div className="flex mt-10">
                        ...
                        <Link to="/login" className="flex-1 w-full">
                          <UIButton className="gray capitalize filled-error flex-1 w-full">
                            Back to Log In
                          </UIButton>
                        </Link>
                      </div>
            ...
            {message && (
              <div
                className={`core-alert flex items-center ${!message?.isError ? "success" : ""}`}
              >
                <div>
